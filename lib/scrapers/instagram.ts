import { extractHashtags, cleanCaption, getInstagramIdFromUrl, validateInstagramUrl } from '../utils';
...
export class InstagramScraper implements Scraper {
  ...
  async scrape(url: string): Promise<ScrapedData | null> {
    try {
      ...
      if (graphqlResult) return this.mapToScrapedData(graphqlResult, url);

      // Fallback to magic parameters method
      const magicResult = await this.fetchWithMagicParams(url);
      if (magicResult) return this.mapToScrapedData(magicResult, url);

      // Final fallback to oEmbed
      const oembedResult = await this.fetchWithOEmbed(url);
      if (oembedResult) return this.mapToScrapedData(oembedResult, url);
  ...
  private mapToScrapedData(data: ReelData, url: string): ScrapedData {
    return {
      url: url,
